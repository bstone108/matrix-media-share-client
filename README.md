...
It publishes GitHub releases from `v*` tags or `workflow_dispatch` after every platform build succeeds. Only those real releases Developer ID sign (hardened runtime + timestamp), notarize, and staple the macOS `.app`, the `.dmg`, and the zip. The zip of the `.app` is the notary vehicle (`ditto -c -k --keepParent`); the ticket is stapled onto the `.app`, and that stapled `.app` is what ships in the release zip. Publish also attaches Sparkle appcasts (`appcast-macos-arm64.xml` and `appcast-macos-x86_64.xml`) whose zip enclosures were signed on the macOS release jobs with Sparkle `sign_update` and `SPARKLE_ED25519_PRIVATE_KEY`. The matching public key is embedded in the app as `SUPublicEDKey`. Pull-request CI never generates appcasts, never receives that secret, and never notarizes. If the run was started from a `v*` tag, publish fails unless that tag matches `VERSION.txt`.
